...
namespace FastColoredTextBoxNS
{
    public static class EncodingDetector
    {
        ...
        {
            return DetectTextFileEncoding(InputFileStream, _defaultHeuristicSampleSize, out bool uselessBool);
        }
        ...
        public static Encoding DetectUnicodeInByteSampleByHeuristics(byte[] SampleBytes)
        {
            ...
            if (UTF8Validator.IsMatch(potentiallyMangledString))
            {
                ...
                // The "Random" likelihood of any pair of randomly generated characters being one of
                // these "suspicious" character sequences is: 128 / (256 * 256) = 0.2%.
                //
                // In western text data, that is SIGNIFICANTLY reduced - most text data stays in the
                // <127 character range, so we assume that more than 1 in 500,000 of these character
                // sequences indicates UTF-8. The number 500,000 is completely arbitrary - so sue me.
                //
                // We can only assume these character sequences will be rare if we ALSO assume that
                // this IS in fact western text - in which case the bulk of the UTF-8 encoded data
                // (that is not already suspicious sequences) should be plain US-ASCII bytes. This,
                // I arbitrarily decided, should be 80% (a random distribution, eg binary data,
                // would yield approx 40%, so the chances of hitting this threshold by accident in
                // random data are VERY low).
